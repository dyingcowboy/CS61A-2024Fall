...
class FreeChecking(Account):
    ...
    def withdraw(self, amount):
        ...
        if amount > self.max_withdrawal:
            return "Can't withdraw that amount"
        self.balance = self.balance - amount
        if self.count_withdraw > self.free_withdrawals:
...
def without(s, i):
    ...
    if i == 0:
        if s.rest.rest is Link.empty:
            return Link(s.rest.first, Link.empty)
        return Link(s.rest.first, without(s.rest.rest, i-1))
    elif s.rest == Link.empty:
        return Link(s.first)
    else:
        Link.first = s.first
        return Link(Link.first, without(s.rest, i-1))
