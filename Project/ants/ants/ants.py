...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            # BEGIN Problem 8b
            if place.ant.is_container == False and  self.can_contain(place.ant):
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        while not temp_place.is_hive:
            if not temp_place.bees == [] and self.lower_bound <= cur_range <= self.upper_bound:
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        if self.health <= amount:
            amount_now = self.damage + amount
        else:
            amount_now = amount
        bees_copy = self.place.bees[:]
        for bee in bees_copy:
            bee.reduce_health(amount_now)
